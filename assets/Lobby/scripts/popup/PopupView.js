...
(function () {
    ...
    cc.PopupView = cc.Class({
		...
		//	nodeHotlines: cc.Node,
            ...
            nodeMessageTop: cc.Node,
            lbMessageTop: cc.Label,
		...
		//	this.nodeHotlines.active = false;
       ...
       onEnable: function () {
            this.animationHotline.play('openPopup');
        },		
        ...
        init: function () {
			...
			this.animationHotline = this.nodePopup.getComponent(cc.Animation);
		...
		hotlineclick: function () {
            
		//	this.animationHotline.play('openPopup');
			this.nodeHotlines.active = true;
        },
		hotlineclose: function () {
            
			this.animationHotline.play('closePopup');
			this.nodeHotlines.active = false;
        },		
        ...
        showMessage: function (message, code) {
            ...
            //this.showMess(message, code);
             ...
             if (message) {
                this.lbMessageTop.string = message;
            } else if (message) {
                this.lbMessageTop.string = 'Đã có lỗi xảy ra';
            } else {
                this.lbMessageTop.string = 'Đã có lỗi xảy ra';
            }
            this.nodeMessage.opacity = 0;
            this.nodeMessageTop.active = true;
            this.animation.stop('openPopupFade');
            this.animation.play('openPopupFade', 0);
            var delay = 2;
            cc.director.getScheduler().schedule(function () {
                this.nodeMessageTop.active = false;
            }, this, 1, 0, delay, false);

        },
        
        showMessageErr: function (message) {
            if (message) {
                this.lbMessageTop.string = message;
            } else if (message) {
                this.lbMessageTop.string = 'Đã có lỗi xảy ra';
        ...
        isShowPopup: function () {
            return !!( this.nodePopup.active || this.nodeBusy.active || this.nodeMessage);
        },
