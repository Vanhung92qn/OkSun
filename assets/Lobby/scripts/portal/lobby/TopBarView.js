...
(function () {
    cc.TopBarView = cc.Class({
        ...
        properties: {
            ...
            //AnimNodeButtonSelectGame
            aniNodeButtonSelectGame: cc.Animation,

            //button select game
            buttonSelectGames: [cc.Button]
        ...
        updateUILogin: function (enable) {
            this.nodeLogin.forEach(function (node) {
                node.active = enable;
            });
            cc.LobbyController.getInstance().showGroupLoginWithAnimation(enable);
            // chạy auto update balance
        ...
        // hàm tự reload 3s 1 lần update số dư
        autoUpdateBalance: function () {
            this.schedule(function () {
                this.lbiBalance.tweenValueto(cc.BalanceController.getInstance().getBalance());
                console.log("Auto updated balance");
            }, 3);
        },
        ...
        topBarUpdateInfo: function () {
            ...
            //this.lbiBalance.string = loginResponse.Balance;
            ...
            // cc.log("isMinigame ==> " + cc.LobbyController.getInstance().lobbyView.isMiniGame);
            // cc.log("isClickSelectGame ==> " + cc.LobbyController.getInstance().lobbyView.isClickSelectGame);
            // if (!cc.LobbyController.getInstance().lobbyView.isMiniGame) {
            //     if (cc.LobbyController.getInstance().lobbyView.isClickSelectGame < 1) {
            //         if (cc.LobbyController.getInstance().lobbyView.isReloadNodeSelectGame) {
            //             this.aniNodeButtonSelectGame.play('openMenuSelectGame');
            //             this.selectGames(null, 0);
            //         }
            //     }
            // }

            // -1 la luc moi vao game
            if (cc.LobbyController.getInstance().lobbyView.isClickSelectGame == -1) {
                if (!cc.LobbyController.getInstance().lobbyView.isMiniGame) {
                    this.aniNodeButtonSelectGame.play('openMenuSelectGame');
                    this.selectGames(null, 0);
                ...
                if (!cc.LobbyController.getInstance().lobbyView.isMiniGame && cc.LobbyController.getInstance().lobbyView.isReloadNodeSelectGame) {
                    this.aniNodeButtonSelectGame.play('openMenuSelectGame');
                    this.selectGames(null, 0);
                }
            }

            cc.LobbyController.getInstance().lobbyView.isMiniGame = false;
        },
        ...
        selectGames: function (event, index) {
            let i = parseInt(index);
            this.buttonSelectGames.forEach(bt => {
                bt.interactable = true;
            });
            this.buttonSelectGames[i].interactable = false;
            cc.LobbyController.getInstance().lobbyView.selectGames(i);
        }
    });
}).call(this);
