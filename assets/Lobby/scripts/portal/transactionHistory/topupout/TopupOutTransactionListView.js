...
(function () {
    cc.TopupOutTransactionListView = cc.Class({
        "extends": cc.ListView,
        properties: {

        },

        initialize: function (messages, start, end) {
            ...
            //     item.getComponent(cc.TopupTransactionItem).updateItem(this.messages[i], i);
            ...
            for (var i = start; i < end; ++i) {
                ...
                var item = cc.instantiate(this.itemTemplate);
                this.content.addChild(item);
                item.setPosition(0, -item.height * (0.5 + j) - this.spacing * (j + 1));
                item.getComponent(cc.TopupOutTransactionItem).updateItem(this.messages[i], i);
                this.items.push(item);
                j++;
            }
            this.rootContentY = this.content.y;
            ...
            for (var i = 0; i < items.length; ++i) {
                ...
                if (isDown) {
                    ...
                    if (viewPos.y < -buffer && items[i].y + offset < 0) {
                        ...
                        var item = items[i].getComponent(cc.TopupOutTransactionItem);
                    ...
                    if (viewPos.y > buffer && items[i].y - offset > -this.content.height) {
                        items[i].y  = (items[i].y - offset);
                        item = items[i].getComponent(cc.TopupOutTransactionItem);
