...
(function () {
    cc.TopupTransactionListView = cc.Class({
        "extends": cc.ListView,
        properties: {
        ...
        update: function (dt) {
            ...
            for (var i = 0; i < items.length; ++i) {
                console.log("Item " + i + " position: " + items[i].y, items[i]);
                var viewPos = this.getPositionInView(items[i]);
                if (isDown) {
                    // if away from buffer zone and not reaching top of content
                    if (viewPos.y < -buffer && items[i].y + offset < 0) {
                        items[i].y  = (items[i].y + offset);
                        var item = items[i].getComponent(cc.TopupTransactionItem);
                    ...
                    if (viewPos.y > buffer && items[i].y - offset > -this.content.height) {
                        items[i].y  = (items[i].y - offset);
                        item = items[i].getComponent(cc.TopupTransactionItem);
