...
(function () {
    cc.AQSpinView = cc.Class({
        extends: cc.SpinViewBase,
        properties: {
            spinColumnViews: [cc.AQSpinColumnView],
            //anh nen cua phong
            spriteBGRoom: cc.Sprite,
            //gia tri dat cuoc
            lbBetVal: cc.Label,
            nodeSpriteTry: cc.Node,
            fastSpinSkeleton: sp.Skeleton,
        ...
        updateFastSpinSke: function(){
            this.isFastSpin = !this.isFastSpin;
            // cc.log('is Fast ===> ' + this.isFastSpin );
            let ske = "IDLE";
            if(this.isFastSpin) ske = "FAST"; 
            this.fastSpinSkeleton.setAnimation(0,ske,true);
        },
        ...
        playEffect: function (spinData) {
            ...
            this.scheduler.schedule(function () {
                ...
                if (cc.FreeSpinController.getInstance().getStateFreeSpin() === false && spinData.TotalFreeSpin > 0) {
                    ...
                    cc.EffectController.getInstance().playEffect(cc.EffectType.FREE_SPIN, spinData.PaylinePrize, self.isFastSpin ? slotsConfig.TIME_TWEEN_MONEY_FAST : null);
        ...
        showLineView: function(event,index){
            let i = parseInt(index);
            cc.PayLinesController.getInstance().hideAllLines();
            cc.PayLinesController.getInstance().payLinesView.showLineID(i);
            this.scheduler.schedule(function () {
                cc.PayLinesController.getInstance().payLinesView.stopGameEffect();
                cc.PayLinesController.getInstance().hideAllLines();
            }, this, 0, 0, 1, false);
        }
