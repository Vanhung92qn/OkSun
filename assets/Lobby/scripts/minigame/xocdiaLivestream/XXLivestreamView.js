...
(function () {
    cc.XXLivestream = cc.Class({
        ...
        reconnect: function () {
            this.lastTimeReconnect = (new Date()).getTime();
            this.xxHub.connect(this, cc.HubName.XocDiaLivestreamHub, this.connectionToken, true);
        },
        ...
        onSlotsNegotiateResponse: function (response) {
            ...
            this.xxHub.connect(this, cc.HubName.XocDiaLivestreamHub, response.ConnectionToken);
        },
                ...
                mArray.map(m => {
                    switch (m.M) {
                        ...
                        case cc.MethodHubOnName.SESSION_INFO:
                            ...
                            //console.log(m.A[0]);
                            cc.XXLivestreamController.getInstance().updateInfo(m.A[0], m.A[0].Phrase, m.A[0].Elapsed);
                            cc.XXLivestreamController.getInstance().updateInput(m.A[0].Phrase);
                            break;
                        //Lich su choi
                        ...
                        case cc.MethodHubOnName.START_ACTION_TIMER:
                            var data = m.A;
                            cc.XXLivestreamController.getInstance().updateInfo(data[0], data[2], data[1]);
                            ...
                            cc.XXLivestreamController.getInstance().updateResult(null, data[0].Result, data[0].Result.ChipsData, data[2],null,data[0]);
                        ...
                        case cc.MethodHubOnName.UPDATE_CONNECTION_STATUS:
                            cc.XXLivestreamController.getInstance().updateConnectionStatus(m.A);
                            break;

                        //cập nhật lại trạng thái player
                        case cc.MethodHubOnName.UPDATE_PLAYER_STATUS:
                            cc.XXLivestreamController.getInstance().updatePlayerStatus(m.A[0]);
                            break;
                        ...
                        case cc.MethodHubOnName.JOIN_GAME:
                            ...
                            var data = m.A[0];
                            var info = m.A[1];
                            cc.XXLivestreamController.getInstance().updateInfoCurrPlayer(data.Account);
                            cc.PopupController.getInstance().hideBusy();
                            break;
                        ...
                        case cc.MethodHubOnName.OPEN_PLATE_NOW:
                            cc.XXLivestreamController.getInstance().updateResult(null, m.A[0], m.A[1], cc.XXLivestreamState.OPEN_PLATE, true);
                            cc.XXLivestreamController.getInstance().updateInput(cc.XXLivestreamState.OPEN_PLATE);
                            break;
