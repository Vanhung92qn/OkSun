    ...
    cc.TaiXiuHistoryItem = cc.Class({
        ...
        updateItem: function(item, itemID) {
           ...
           // this.lbResult.string = item.Result;
            ...
            var myArray = item.Result.split("-");
            if(myArray[2]){
            	var myArray2 = myArray[2].split(" ");
            	if(myArray2){
            		var totaldice =  parseInt(myArray[0])+parseInt(myArray[1])+parseInt(myArray2[0]);
            		var resulttxt = '(';
            		if(totaldice > 10){
            			resulttxt += 'Tài-';
            		}else{
            		...
            		this.lbResult.string = myArray[0]+'-'+myArray[1]+'-'+myArray2[0]+' '+resulttxt;
            	}
            }
            this.lbRefund.string = cc.Tool.getInstance().formatNumber(item.Award);
		...
		//		var tongtiennhan = cc.Tool.getInstance().formatNumber(item.Award);
		//		this.lbWin.string = cc.Tool.getInstance().formatNumber(item.Award);
				...
				//this.lbResult.string = item.Result;
				this.lbWin.string = cc.Tool.getInstance().formatNumber(item.Award);
				var tongtiennhan = cc.Tool.getInstance().formatNumber(item.Refund);
