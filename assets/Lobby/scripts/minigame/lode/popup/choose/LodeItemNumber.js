(function () {
    cc.LodeItemNumber = cc.Class({
        ...
        setNumber: function (number) {
            ...
            if (typeBet == cc.LodeType.BaCang || typeBet == cc.LodeType.Lo3So) {
                strNumber = (number < 10) ? "00" + number : ((number < 100) ? "0" + number : number);
            }
            ...
            this.lbNumberHighlight.string = strNumber;
        ...
        onChoose: function () {
            ...
            if (this.isChoose) {
                ...
                if (!cc.LodeController.getInstance().isMaxChoose()) {
                    ...
                    this.lbNumber.node.active = !this.isChoose;
                    this.lbNumberHighlight.node.active = this.isChoose;
                    cc.LodeController.getInstance().setNumberChooses(this.numberValue);
                }
                else {
                    this.spriteNumber.spriteFrame = this.sfUnChoose;
                    cc.LodeController.getInstance().getChooseView().showNotifyWarningChooseNumber();
                }
            } else {
                this.spriteNumber.spriteFrame = (this.isChoose) ? this.sfChoose : this.sfUnChoose;
                this.lbNumber.node.active = !this.isChoose;
