...
(function () {
  cc.TaiXiuSicboInputBetView = cc.Class({
    ...
    addSuccess: function () {
      if (BetSelf) {
        let spr = BetSelf.betNode
        .getChildByName("spr");
        
        let lbXengBet = spr
          .getChildByName("lb_bet")
          .getComponent(cc.Label);
        if (BetSelf.betValue > 0) {
          spr.active = true;
        }
        else{
          spr.active = false;
    ...
    onLoad: function () {
      ...
      for (let i of this.nodeBots) {
        let spr = i.getChildByName("Avartar")
        .getChildByName("New Node");
        var lbWin = spr.getChildByName("lb win");

        if (lbWin) {
          lbWin.getComponent(cc.Label).string = "";
          lbWin.parent.active = false;
        }
      }
      ...
      let toggle = this.chips.getComponent(cc.ToggleContainer);
      for (let i = 0; i < toggle.toggleItems.length; i++) {
        toggle.toggleItems[i].node.on("toggle", () => {
            this._onTabChange();
        });
        this._onTabChange();
    }
    ...
    startJoinRoom: function () {
      this.nodeUsers
        ...
        .setValue(Math.floor(Math.random() * (500 - 100 + 1) + 100), 3000);
      ...
      if (this.sttBot > this.nodeBots.length - 1) {
        ...
        this.scheduler.schedule(
          function () {
            ...
            let botBet = 0;
            let moneyString = bot.getChildByName("New Label").getComponent(cc.Label).string;
            let unit = moneyString.slice(-1);
            let money = Number(moneyString.slice(0,moneyString.length - 1));
            if (unit == 'M' || unit == 'm') {
              botBet = money * 1000000;
            }
            else{
              botBet = money * 1000;
            }
            // let botBet = Number(
            //   bot.getComponentInChildren("LabelIncrement").string.replaceAll(",", "")
            // );
            ...
            if (botBet < rndValue && this.ignoreBot != rand2) {
              ...
              this.id2 = setTimeout(
                function () {
                  ...
                  let nodelabel = this.nodeBots[this.ignoreBot]
                  .getComponentInChildren("LabelIncrement");
                  nodelabel.formatCurrencyUnit = true;
                  this.nodelabel
                    .setValue(
                      Math.floor(
            ...
            nodelabel.formatCurrencyUnit = true;
            nodelabel
              .setValue(Math.abs(botBet - rndValue));
      ...
      this.sttBotIdx = setTimeout(
        function () {
          ...
          this.nodeBots[this.sttBot]
            .getComponentInChildren("Avatar")
            .setAvatar(this.sprBotAvatars[rndSpr]);
            let nodelabel = this.nodeBots[this.sttBot]
            .getComponentInChildren("LabelIncrement");
      nodelabel.formatCurrencyUnit = true;
      nodelabel
            .setValue(
    ...
    startEffectUserWin: function (money) {
      this.lbUserWin.node.parent.active = true;
      this.lbUserWin.node.active = true;
      this.nodeUserWin.active = true;
      this.lbUserWin.string = "+" + cc.Tool.getInstance().formatMoney(money);

      setTimeout(
        function () {
          this.lbUserWin.node.parent.active = false;
          this.lbUserWin.node.active = false;
          this.nodeUserWin.active = false;
        }.bind(this),
        6000
      );
    },
          ...
          for (var i = 0; i < rnd; i++) {
            ...
            try {
              ...
              if (bot) {
                ...
                if (realAmount > 0) {
                  let spr = bot.getChildByName("Avartar")
        .getChildByName("New Node");
                  let nodeOther = spr.getChildByName("lb win");
                  nodeOther.active = true;
                  nodeOther.parent.active = true;
    ...
    _createBotChipAnimation: function (
      ...
      xeng.setSiblingIndex(this.nodeChipParent.children.length -1);
      this.nodeUsers.setSiblingIndex(this.nodeChipParent.children.length);
    ...
    onClickNextChip() {
      this.chipsIdx++;
      if (this.chipsIdx > 2) this.chipsIdx = 2;
      this.chips.x = -this.chipsIdx * 362;
    },
    onClickPrevChip() {
      this.chipsIdx--;
    ...
    _onTabChange:function(){
      let betChecked = this.chips
        .getComponent(cc.ToggleContainer)
        .toggleItems.find((e) => e.isChecked == true).node;
      let node_glow = betChecked.getChildByName('glow').getChildByName('glow_scale');
      cc.tween(node_glow)
                            .repeatForever(
                                cc.tween().sequence(
                                    cc.tween().to(0.3, { scale: 1 }, { easing: cc.easing.sineOut }),
                                    cc.tween().to(0.3, { scale: 0.95 }, { easing: cc.easing.sineOut })))
                            .start();
    }
  });
}).call(this);
