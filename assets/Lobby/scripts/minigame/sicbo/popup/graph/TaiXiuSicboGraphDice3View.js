...
(function () {
    cc.TaiXiuSicboGraphDice3View = cc.Class({
        ...
        drawDice1: function (list) {
            ...
            list.forEach(function (item) {
                self.createNode(self.drawing1, item.FirstDice, index,self.spriteDice[0]);
                index++;
            });	
        ...
        drawDice2: function (list) {
            ...
            list.forEach(function (item) {
                self.createNode(self.drawing2, item.SecondDice, index,self.spriteDice[1]);
                index++;
            });
        ...
        drawDice3: function (list) {
            ...
            list.forEach(function (item) {
                self.createNode(self.drawing3, item.ThirdDice, index,self.spriteDice[2]);
                index++;
            });
        ...
        createNode: function (drawing, dice, colIndex,spriteDice) {
            //toa do X
            ...
            nodeView = cc.instantiate(this.nodeDice1Temp);
            nodeView.parent = this.nodeParent;
            nodeView.position = cc.v2(posX, posY);
            var self = this;
            self.drawPoints.push(cc.v2(posX, posY));
            nodeView.getComponent(cc.Sprite).spriteFrame = spriteDice;
        },
