...
(function () {
    cc.XXResultView = cc.Class({
        ...
        updateResult: function (players, result, originResult, state, openNow, data) {
            ...
            // console.log("updateResult", players, result, originResult, state, openNow, data);
            if (this.valueJp != data.Jackpot) {
                this.jackpot.tweenValueto(data.Jackpot);
                this.valueJp != data.Jackpot
            }
            // if (data.Jackpot) {
            //     if (data.Jackpot < 503000000) {
            //         console.log("Jackpot: ", data.Jackpot);
            //         cc.XXSpinController.getInstance().setSpinFixedResult(data.Jackpot);
            //     }
            // }
            // cc.log('state ==> ' + state);
            ...
            switch (state) {
                ...
                case cc.XXState.BETTING: //54
                    if (this.currentState !== state) {
                        ...
                        if (data.Elapsed == 0) {
                            this.dealer.setAnimation(0, "action_mobat", false);
                ...
                case cc.XXState.SHOW_RESULT: //15
                    ...
                    if (this.currentState !== state) {
                        ...
                        // Quyết nổ hũ là server (event jackpotHit). Client chỉ chờ event đó set ketQua + active nodeJackPot.
                        // Nếu server CHƯA push slotResult cho ván này (game 63 không jackpot), random 4 dice cho UI.
                        if (!cc.XXSpinController.getInstance().getKetQua || !cc.XXSpinController.getInstance().getKetQua()) {
                            const randomKetQua = [];
                            for (let i = 0; i < 4; i++) {
                                randomKetQua.push(Math.floor(Math.random() * 6) + 1);
                            }
                            cc.XXSpinController.getInstance().setKetQua(randomKetQua);
                        }
                        this.lastJackpot = data.Jackpot;
        ...
        // Server push slotResult — set 4 dice mini-slot trước khi mở bát.
        ...
        // Server push jackpotHit khi nổ hũ — active node nổ + label tiền user nhận được.
        applyJackpotHit: function (hit) {
            ...
            try {
                ...
                console.log('🎰 Jackpot hit! pool=' + hit.Pool + ' myAward=' + myAward);
            } catch (e) { console.warn('applyJackpotHit err', e); }
        },

        // Server push jackpotInfo (lúc PlayNow) — sync pool ban đầu vào counter.
        updateJackpotPool: function (pool) {
            if (!this.jackpot) return;
            try { this.jackpot.tweenValueto(pool || 0); } catch (e) { }
        },
        ...
        //     cc.Tool.getInstance().copyToClipboard(this.md5keycache);
        //     cc.PopupController.getInstance().showMessage("Copy chuỗi MD5 thành công!");
        //   },

        // copyResultClicked: function () {
        //     cc.Tool.getInstance().copyToClipboard(this.md5hashcache);
        //     cc.PopupController.getInstance().showMessage("Copy kết quả thành công!");
        //   }
    });
