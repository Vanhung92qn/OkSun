...
(function () {
  ...
  XXController = (function () {
    ...
    instance = void 0;
    // tạo 1 biến để lưu trữ type history
    ...
    XXController.prototype.getTypeHistory = function () {
      return this.type;
    };

    XXController.prototype.setTypeHistory = function (type) {
      this.type = type;
    };
    ...
    XXController.prototype.updateChip = function (accID, chip) {
      // console.log("updateChip", accID, chip);
    ...
    XXController.prototype.setNodeMain = function (active) {
      // đợi 2s để chắc chắn rằng nodeMain đã được khởi tạo
      setTimeout(() => {
        // console.log(this.xxInfoView, this.xxInfoView.setMain);
        if (this.xxInfoView && this.xxInfoView.setMain) {
          this.xxInfoView.setMain(active);
        }
      }, 2000);
    };

    //set ket qua cua player
    XXController.prototype.playerResultUI = function (
      playerIndex,
      isWin,
      amount
    ) {
    ...
    //set ket qua cua player
    XXController.prototype.setAnimationTimeBet = function (aniName) {
      return this.xxInfoView.playAnimationCuoc(aniName);
    };
    ...
    XXController.prototype.summaryPlayer = function (total) {
      ...
      const numbers = offset.map((o) => {
        const val = Math.sin((t + o) * 0.5) * 50 + 750;
        return Math.floor(val);
      });
      return this.xxInfoView.summaryPlayer(numbers[0] + 100);
    };
    // HubOn - vipPlayer
    ...
    //RESULT
    XXController.prototype.updateResult = function (
      players,
      result,
      originResult,
      state,
      ...
      return this.xxResultView.updateResult(
        players,
        result,
        originResult,
        state,
    ...
    XXController.prototype.draw = function (list) {
      return this.xxSoiCauView.draw3(list);
    };
