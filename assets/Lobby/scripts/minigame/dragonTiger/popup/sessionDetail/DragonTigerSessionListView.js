...
(function () {
    cc.DragonTigerSessionListView = cc.Class({
        "extends": cc.ListView,
        properties: {
        ...
        initialize: function (messages) {
            ...
            this.content.weight = countMessage * (this.itemTemplate.weight + this.spacing) + this.spacing;
            ...
            for (var i = 0; i < spawnCountReal; ++i) {
                ...
                item.setPosition( +item.weight * (0.5 + i) - this.spacing * (i + 1), 0);
                item.getComponent(cc.DragonTigerSessionItem).updateItem(this.messages[i], i);
                this.items.push(item);
            }

            this.rootContentX = this.content.x;
        },
            ...
            for (var i = 0; i < items.length; ++i) {
                ...
                if (isDown) {
                    ...
                    if (viewPos.y < -buffer && items[i].y + offset < 0) {
                        ...
                        var item = items[i].getComponent(cc.TaiXiuSessionDetailItem);
                    ...
                    if (viewPos.y > buffer && items[i].y - offset > -this.content.height) {
                        items[i].y  = (items[i].y - offset);
                        item = items[i].getComponent(cc.TaiXiuSessionDetailItem);
